...
namespace lit
{
    ...
    namespace AST
    {
        ...
        struct Scanner
        {
            ...
            public:
                void init(State* state, const char* file_name, const char* source)
                {
                    this->line = 1;
                    this->start = source;
                    this->current = source;
        ...
        struct Parser
        {
            ...
            public:
                ...
                bool parse(const char* file_name, const char* source, Expression::List& statements)
                {
                    Compiler compiler;
                    Expression* statement;
                    this->had_error = false;
    ...
    */
     ...
     */
    Module* lit_compile_module(State* state, String* module_name, char* code);
    Module* lit_get_module(State* state, const char* name);

    InterpretResult lit_internal_interpret(State* state, String* module_name, char* code);
    ...
    size_t lit_write_string(FILE* file, String* string);
    ...
    String* lit_read_string(State* state, FILE* file);

    void lit_init_emulated_file(EmulatedFile* file, const char* source);

    uint8_t lit_read_euint8_t(EmulatedFile* file);
    uint16_t lit_read_euint16_t(EmulatedFile* file);
    uint32_t lit_read_euint32_t(EmulatedFile* file);
    double lit_read_edouble(EmulatedFile* file);
    String* lit_read_estring(State* state, EmulatedFile* file);

    void lit_save_module(Module* module, FILE* file);
    Module* lit_load_module(State* state, const char* input);
    ...
    */
    ...
    static void load_chunk(State* state, EmulatedFile* file, Module* module, Chunk* chunk);
    ...
    static Function* load_function(State* state, EmulatedFile* file, Module* module)
    {
        ...
        for(size_t i = 0; i < chunk->constants.m_count; i++)
        {
            ...
            if(Object::isObject(constant))
            {
                ...
                switch(type)
                {
                    case Object::Type::String:
                    {
                        lit_write_string(file, Object::as<String>(constant));
                        break;
                    }
            ...
            else
            {
                ...
                lit_write_double(file, Object::toNumber(constant));
            }
        }
    }
    ...
    static void load_chunk(State* state, EmulatedFile* file, Module* module, Chunk* chunk)
    {
        ...
        for(i = 0; i < count; i++)
        {
            ...
            if(type == 0)
            {
                chunk->constants.m_values[i] = Object::toValue(lit_read_edouble(file));
            }
            else
            {
        ...
        if(!disabled)
        {
            ...
            for(i = 0; i < module->private_count; i++)
            {
                if(privates->m_values[i].key != nullptr)
                {
                    ...
                    lit_write_uint16_t(file, (uint16_t)Object::toNumber(privates->m_values[i].value));
                }
            }
        }
    ...
    Module* lit_load_module(State* state, const char* input)
    {
        ...
        {
            module = Module::make(state, lit_read_estring(state, &file));
            privates = &module->private_names->values;
            ...
            for(i = 0; i < privates_count; i++)
            {
                ...
                if(enabled)
                {
                    ...
                    privates->set(name, Object::toValue(lit_read_euint16_t(&file)));
                }
            }
    ...
    InterpretResult lit_interpret(State* state, const char* module_name, char* code)
    {
        return lit_internal_interpret(state, String::copy(state, module_name, strlen(module_name)), code);
    }
    ...
    static char* read_source(State* state, const char* file, char** patched_file_name)
    {
        clock_t t;
        char* file_name;
        char* source;
    ...
    */
    ...
    static bool compile_and_interpret(VM* vm, String* modname, char* source)
    {
        Module* module;
    ...
    static Value objmethod_file_writebyte(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        rt = lit_write_uint8_t(((FileData*)LIT_EXTRACT_DATA(vm, instance))->handle, byte);
        return Object::toValue(rt);
    }
    ...
    static Value objmethod_file_writeshort(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        rt = lit_write_uint16_t(((FileData*)LIT_EXTRACT_DATA(vm, instance))->handle, shrt);
        return Object::toValue(rt);
    }
    ...
    static Value objmethod_file_writenumber(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        rt = lit_write_uint32_t(((FileData*)LIT_EXTRACT_DATA(vm, instance))->handle, num);
        return Object::toValue(rt);
    }
    ...
    static Value objmethod_file_writebool(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        rt = lit_write_uint8_t(((FileData*)LIT_EXTRACT_DATA(vm, instance))->handle, (uint8_t)value ? '1' : '0');
        return Object::toValue(rt);
    }
    ...
    static Value objmethod_file_readbyte(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        (void)argc;
        (void)argv;
        return Object::toValue(lit_read_uint8_t(((FileData*)LIT_EXTRACT_DATA(vm, instance))->handle));
    }
    ...
    static Value objmethod_file_readshort(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        (void)argc;
        (void)argv;
        return Object::toValue(lit_read_uint16_t(((FileData*)LIT_EXTRACT_DATA(vm, instance))->handle));
    }
    ...
    static Value objmethod_file_readnumber(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        (void)argc;
        (void)argv;
        return Object::toValue(lit_read_uint32_t(((FileData*)LIT_EXTRACT_DATA(vm, instance))->handle));
    }
    ...
    static Value objmethod_file_readbool(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        (void)argc;
        (void)argv;
        return BOOL_VALUE((char)lit_read_uint8_t(((FileData*)LIT_EXTRACT_DATA(vm, instance))->handle) == '1');
    }
    ...
    static Value objmethod_file_readstring(VM* vm, Value instance, size_t argc, Value* argv)
    {
        ...
        String* string = lit_read_string(vm->m_state, data->handle);
...
int main(int argc, const char* argv[])
{
    ...
    for(i = 1; i < argc; i++)
    {
        ...
        else if(match_arg(arg, "-e", "--eval"))
        {
            ...
            if(dump)
            {
                module = lit_compile_module(state, lit::String::intern(state, module_name), source);
                if(module == nullptr)
                {
                    break;
                }
